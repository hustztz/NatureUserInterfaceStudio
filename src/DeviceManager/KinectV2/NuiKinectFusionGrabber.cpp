#include "NuiKinectFusionGrabber.h"
...
NuiKinectFusionGrabber::NuiKinectFusionGrabber()
	: m_threadOn(false)
	, m_threadPause(false)
	, m_pCache(NULL)
	, m_pVolume(NULL)
...
bool NuiKinectFusionGrabber::initializeKinectFusion()
{
	HRESULT hr = S_OK;
	m_threadOn = false;
...
/// </summary>
/// <returns>S_OK on success, otherwise failure code</returns>
HRESULT NuiKinectFusionGrabber::ResetReconstruction()
{
...
/// </summary>
HRESULT NuiKinectFusionGrabber::CameraTrackingOnly()
{
...
HRESULT NuiKinectFusionGrabber::ProcessPointCloud(NuiCompositeFrame* pFrame)
{
	assert(pFrame);
	TIMESPAN currentFrameTime = pFrame->GetTimeStamp();
	...
	if( pFrame->ReadPointCloud(&cachedPointCloud) )
	{
		return S_FALSE;
	}
	...
	else
	{
		...
		if (SUCCEEDED(hr))
		{
			...
			pFrame->SetCameraPose(calculatedCameraPose);
		}
	}
	...
	BGRQUAD* pFusionImageBuffer = pFrame->AllocateFusionImage(m_pShadedSurface->width, m_pShadedSurface->height);
...
bool NuiKinectFusionGrabber::readMesh(NuiFusionMesh* pMesh)
{
	if(!m_pVolume || !pMesh)
		return false;
...
void NuiKinectFusionGrabber::runThread ()
{
	while (true)
	{
		if (!m_threadOn)
		...
		NuiCompositeFrame* pFrame = m_pCache->GetLatestCompoundFrame();
		if (!pFrame)
			continue;
...
void NuiKinectFusionGrabber::startThread ()
{
	if(m_pVolume && !m_threadOn)
	{
		ResetReconstruction();

		m_Thread.reset (new boost::thread (boost::bind (&NuiKinectFusionGrabber::runThread, this)));
		m_threadOn = true;
...
void NuiKinectFusionGrabber::pauseThread()
{
	m_threadPause = true;
}

void NuiKinectFusionGrabber::stopThread ()
{
	m_threadOn = false;
	m_threadPause = false;
	if(m_Thread)
...
void NuiKinectFusionGrabber::updateCache(NuiDeviceCacheImpl* pCache)
{
	m_pCache = pCache;
}
