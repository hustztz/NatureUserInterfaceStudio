...
NuiHashingOpenCLChunkGrid::~NuiHashingOpenCLChunkGrid()
{
	ReleaseBuffers();
}

void NuiHashingOpenCLChunkGrid::AcquireBuffers()
{
	m_SDFBlockOutput = new NuiOpenCLSDFBlock[m_maxNumberOfSDFBlocksIntegrateFromGlobalHash];
	m_SDFBlockDescOutput = new NuiOpenCLSDFBlockDesc[m_maxNumberOfSDFBlocksIntegrateFromGlobalHash];
	...
	NUI_CHECK_CL_ERR(err);
	m_SDFBlockOutputCL = NuiGPUMemManager::instance().CreateBufferCL(context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, sizeof(NuiOpenCLSDFBlock)*m_maxNumberOfSDFBlocksIntegrateFromGlobalHash, m_SDFBlockOutput, &err);
	NUI_CHECK_CL_ERR(err);
	m_SDFBlockInputCL = NuiGPUMemManager::instance().CreateBufferCL(context, CL_MEM_READ_WRITE, sizeof(NuiOpenCLSDFBlock)*m_maxNumberOfSDFBlocksIntegrateFromGlobalHash, NULL, &err);
	NUI_CHECK_CL_ERR(err);
...
void NuiHashingOpenCLChunkGrid::ReleaseBuffers()
{
...
void NuiHashingOpenCLChunkGrid::reset()
{
...
SgVec3i NuiHashingOpenCLChunkGrid::worldToChunks(const SgVec3f& posWorld) const {
	SgVec3f p;
...
void NuiHashingOpenCLChunkGrid::streamOutToCPUAll(UINT nStreamOutParts)
{
...
UINT NuiHashingOpenCLChunkGrid::streamOutToCPU(UINT nStreamOutParts, const SgVec3f& posCamera, float radius, bool useParts)
{
...
UINT NuiHashingOpenCLChunkGrid::streamOutToCPUPass0GPU(UINT nStreamOutParts, const SgVec3f& posCamera, float radius, bool useParts )
{
	if(!m_pHashingSDF)
		return 0;
	...
	if (nSDFBlockDescs != 0) {
		...
		NUI_CHECK_CL_ERR(err);
		memcpy_s(m_SDFBlockDescOutput, nSDFBlockDescs * sizeof(NuiOpenCLSDFBlockDesc), SDFBlockDescOutputMapper, nSDFBlockDescs * sizeof(NuiCLHashEntry));
		...
		NuiOpenCLSDFBlock* SDFBlockOutputMapper = (NuiOpenCLSDFBlock*)clEnqueueMapBuffer(
			queue,
			m_SDFBlockOutputCL,
			CL_TRUE,//blocking
			CL_MAP_READ,
			0,
			nSDFBlockDescs * sizeof(NuiOpenCLSDFBlock),
			0,
			NULL,
			NULL,
			&err
		...
		NUI_CHECK_CL_ERR(err);
		memcpy(m_SDFBlockOutput, SDFBlockOutputMapper, nSDFBlockDescs * sizeof(NuiOpenCLSDFBlock));
		err = clEnqueueUnmapMemObject(
			queue,
			m_SDFBlockOutputCL,
...
bool NuiHashingOpenCLChunkGrid::isValidChunk(const SgVec3i& chunk) const	{
...
UINT NuiHashingOpenCLChunkGrid::linearizeChunkPos(const SgVec3i& chunkPos) const {
...
void NuiHashingOpenCLChunkGrid::streamOutToCPUPass1CPU(UINT nStreamedBlocks)
{
	const NuiHashingSDFConfig& hashParams = m_pHashingSDF->getConfig();
...
SgVec3f NuiHashingOpenCLChunkGrid::chunkToWorld(const SgVec3i& posChunk) const
{
	SgVec3f res;
...
float NuiHashingOpenCLChunkGrid::getGridRadiusInMeter() const
{
...
void NuiHashingOpenCLChunkGrid::streamInToGPUAll()
{
...
UINT NuiHashingOpenCLChunkGrid::streamInToGPUAll(const SgVec3f& posCamera, float radius, bool useParts )
{
...
void NuiHashingOpenCLChunkGrid::streamInToGPUChunk(const SgVec3i& chunkPos )
{
	unsigned int nStreamedBlocks = 1;
...
void NuiHashingOpenCLChunkGrid::streamInToGPUChunkNeighborhood(const SgVec3i& chunkPos, int kernelRadius )
{
...
UINT NuiHashingOpenCLChunkGrid::streamInToGPU(const SgVec3f& posCamera, float radius, bool useParts)
{
...
SgVec3i	NuiHashingOpenCLChunkGrid::meterToNumberOfChunksCeil(float f) const {
...
SgVec3i NuiHashingOpenCLChunkGrid::delinearizeChunkIndex(unsigned int idx)	{
...
bool NuiHashingOpenCLChunkGrid::isChunkInSphere(const SgVec3i& chunk, const SgVec3f& center, float radius) const {
...
UINT NuiHashingOpenCLChunkGrid::streamInToGPUPass0CPU( const SgVec3f& posCamera, float radius, bool useParts )
{
	// OpenCL command queue and device
	cl_int           err = CL_SUCCESS;
	cl_command_queue queue = NuiOpenCLGlobal::instance().clQueue();

	const unsigned int blockSize = sizeof(NuiOpenCLSDFBlock)/sizeof(int);
	const unsigned int descSize = sizeof(NuiOpenCLSDFBlockDesc)/sizeof(int);
			...
			for (int z = startChunk[2]; z <= endChunk[2]; z++) {
				...
				if (m_grid[index] != NULL && m_grid[index]->isStreamedOut()) // As been allocated and has streamed out blocks
				{
					if (isChunkInSphere(delinearizeChunkIndex(index), posCamera, radius)) // Is in camera range
					{
						...
						NUI_CHECK_CL_ERR(err);
						memcpy_s(SDFBlockDescOutputMapper, nBlock * sizeof(NuiCLHashEntry), &(m_grid[index]->getSDFBlockDescs()[0]), nBlock * sizeof(NuiOpenCLSDFBlockDesc));
						err = clEnqueueUnmapMemObject(
							queue,
							m_SDFBlockDescOutputCL,
							(void*)SDFBlockDescOutputMapper,
						...
						NuiOpenCLSDFBlock* SDFBlockOutputMapper = (NuiOpenCLSDFBlock*)clEnqueueMapBuffer(
							...
							CL_MAP_WRITE,
							0,
							nBlock * sizeof(NuiOpenCLSDFBlock),
							0,
							NULL,
							NULL,
							&err
						...
						NUI_CHECK_CL_ERR(err);
						memcpy(SDFBlockOutputMapper, &(m_grid[index]->getSDFBlocks()[0]), nBlock * sizeof(NuiOpenCLSDFBlock));
						err = clEnqueueUnmapMemObject(
							queue,
							m_SDFBlockOutputCL,
							(void*)SDFBlockOutputMapper,
...
void NuiHashingOpenCLChunkGrid::streamInToGPUPass1GPU(UINT nStreamedBlocks)
{
	// Get the kernel
