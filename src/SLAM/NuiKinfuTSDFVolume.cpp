...
NuiKinfuTSDFVolume::NuiKinfuTSDFVolume(const NuiKinfuVolumeConfig& config)
	: m_config(config)
	, m_volumeCL(NULL)
...
NuiKinfuTSDFVolume::~NuiKinfuTSDFVolume()
{
	ReleaseBuffer();
}

const Vector3f& NuiKinfuTSDFVolume::getDimensions() const
{
    return m_config.dimensions;
}

const Vector3i& NuiKinfuTSDFVolume::getResolution() const
{
	return m_config.resolution;
}

const Vector3f NuiKinfuTSDFVolume::getVoxelSize() const
{    
	return m_config.dimensions.array () / m_config.resolution.array().cast<float>();
}

float NuiKinfuTSDFVolume::getTsdfTruncDist () const
{
	return m_tsdf_params.tranc_dist;
}

Vector3i NuiKinfuTSDFVolume::getVoxelWrap() const
{
...
Vector3f NuiKinfuTSDFVolume::getVoxelOffsetSize() const
{
...
void NuiKinfuTSDFVolume::AcquireBuffer(bool bHas_color_volume)
{
	cl_int           err = CL_SUCCESS;
...
void NuiKinfuTSDFVolume::ReleaseBuffer()
{
	if (m_volumeCL) {
...
void NuiKinfuTSDFVolume::clearSlice(const Vector3i&	voxelWrap, const Vector3i&	voxelRange)
{
...
void NuiKinfuTSDFVolume::fetchSlice(const Vector3i&	voxelWrap, const Vector3i&	voxelRange)
{
...
void NuiKinfuTSDFVolume::fetchAndClearY(int yVoxelTrans)
{
...
void NuiKinfuTSDFVolume::fetchAndClearZ(int zVoxelTrans)
{
...
Vector3f NuiKinfuTSDFVolume::shiftVolume(const Vector3f& translation)
{
...
void    NuiKinfuTSDFVolume::Integrate(cl_mem floatDepthsCL, cl_mem colorsCL, cl_mem normalsCL, cl_mem cameraParamsCL, const NuiKinfuTransform& currPos, UINT nWidth, UINT nHeight)
{
...
void	NuiKinfuTSDFVolume::incrementVolume(cl_mem floatDepthsCL, cl_mem colorsCL, cl_mem normalsCL, cl_mem cameraParamsCL, const NuiKinfuTransform& currPos, UINT nWidth, UINT nHeight)
{
...
bool	NuiKinfuTSDFVolume::evaluateVolume(
...
bool NuiKinfuTSDFVolume::Volume2CLVertices(NuiCLMappableData* pCLData)
{
	assert(pCLData);
	if(!pCLData)
		return false;
...
bool NuiKinfuTSDFVolume::Volume2CLMesh(NuiCLMappableData* pCLData)
{
	assert(pCLData);
	if(!pCLData)
		return false;
...
SgVec3f NuiKinfuTSDFVolume::getNodeCoo (int x, int y, int z)
{
	return SgVec3f(
...
bool	NuiKinfuTSDFVolume::Volume2Mesh(NuiMeshShape* pMesh)
{
	assert(pMesh);
	if(!pMesh)
		return false;
